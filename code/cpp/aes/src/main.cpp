...
int main(const int argc, const char** argv)
{
    ...
    static const std::optional<std::vector<uint8_t>> iv_vector = std::vector<uint8_t>{0xff, 0xff, 0xff, 0xff, 0xff, 0xff,0xff, 0xff, 0xff, 0xff, 0xff, 0xff,0xff, 0xff, 0xff, 0xff}; //= aes::cvt::get_vector_from_string<uint8_t, std::string_view>(iv);
    std::vector<uint8_t> plaintext_vector = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55,
                                             0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb,
                                             0xcc, 0xdd, 0xee, 0xff}; //aes::cvt::get_vector_from_string<uint8_t, std::string>(plaintext);
    AES_DEBUG("plaintext_vector: {}", std::string(plaintext_vector.begin(), plaintext_vector.end()))
    std::vector<uint8_t> key_vector = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a,
                                        0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
    ...
    aes::api::encrypt_file(plaintext_file, encrypted_file, key, iv_vector, aes::pad::Paddings::NO_PADDING, aes::mod::Modes::CFB, aes::AES::AES_256);

    const std::string& encrypted_data = aes::fm::FileManager::get_file_data3(encrypted_file);

    AES_DEBUG("encrypted_data: {}", encrypted_data)

    aes::api::decrypt_file(encrypted_file, deciphered_plaintext_file, key, iv_vector, aes::pad::Paddings::NO_PADDING, aes::mod::Modes::CFB, aes::AES::AES_256);
